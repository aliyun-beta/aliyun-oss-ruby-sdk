...
describe Aliyun::Oss::Client do
  ...
  def stub_api_request(verb, endpoint, response)
    stub_request(verb, endpoint)
      .with(Aliyun::Oss::Utils.hash_slice(response, :query, :body, :headers))
      .to_return(status: 200, body: '', headers: {})
  end

  def assert_aliyun_api(method, request = [], response = {})
    new_host = get_host(response[:bucket], response[:location])
    endpoint = "http://#{new_host}/#{response[:key]}"

    headers = default_headers.merge(response[:headers] || {})
    headers.merge!('Host' => new_host)

    response.merge!(headers: headers)

    stub_request1 = stub_api_request(response[:verb], endpoint, response)
  ...
  it 'should get object meta information' do
    assert_aliyun_api(:bucket_get_meta_object, [
      'sample.txt',
  ...
  it 'should copy object' do
    assert_aliyun_api(:bucket_copy_object, [
      'sample1.txt',
      'oss-sdk-dev-beijing',
      'sample.txt',
